...
exp_data_path = joinpath(uns.examples_path, "..", "docs", "resources", "data")
data = CSV.read(joinpath(exp_data_path, "dji9443-Austins-RANS_Thrust_new01.csv"), DataFrame)

urans_factor = 1/(rho*(RPM/60)^2*(2*R)^4)

ax.plot(data[!, 1]*RPM/60, data[!, 2]*urans_factor, "-",
...
for (sim_run_name, stl, clr, alpha, lbl) in sims_to_plot
    ...
    nsteps_per_rev_sims[sim_run_name] = ceil(Int, 360 / (simdata[2, 1] - simdata[1, 1]))

    # Calculate mean CT and std dev
    roti = 1                # Rotor to average
    nsteps_to_average = nrevs_to_average*nsteps_per_rev_sims[sim_run_name]
    data_to_average = simdata[end-nsteps_to_average:end, 3 + (roti-1)*4 + 1+coli]

    CTmean[sim_run_name] = uns.mean(data_to_average)
    CTstd[sim_run_name] = sqrt(uns.mean((data_to_average .- CTmean[sim_run_name]).^2))
...
for (sim_run_name, _, _, _, _) in sims_to_plot
    ...
    nums      = range(8, 9; length = nsteps_per_rev_sims[sim_run_name]+1)
    nums      = ceil.(Int, nums * nsteps_per_rev_sims[sim_run_name])
...
data = CSV.read(joinpath(exp_data_path, "dji9443-Austins-RANS_loading.csv"),
                                                        DataFrame; skipto=1)

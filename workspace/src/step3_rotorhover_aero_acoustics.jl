#=##############################################################################
# Description
    Computes aero acoustic noise of DJI 9443 simulation and compares results to
    experimental data
=###############################################################################
...
save_ww_path    = joinpath(sims_path, run_name*"-aero_acoustics")
...
if const_solution                       # If constant solution, it overrides to read only the first time step
    ...
    nsteps_per_rev_wopwop = nsteps_per_rev
end
...
# microphoneX   = sph_R*[-sin(anglemic), cos(anglemic), 0]

# Alternative observer definition: Single microphone
# Rmic          = sph_R                # (m) radial distance from rotor hub
...
following lines grab those input files that are formatted for PSU-WOPWOP,
...
save_bpm_path   = joinpath(sims_path, run_name*"-bpm")

# ------------ PARAMETERS --------------------------------------------------
# NOTE: Make sure that these parameters match what was used in the
#       aerodynamic solution

# Rotor geometry (BPM uses independent discretization from the aero solution)
read_polar      = vlm.ap.read_polar2   # What polar reader to use
n_bpm           = 50                   # Number of blade elements for BPM
...
generating the aero acoustic solutions:
=#
# These parameters will be used for plotting
nblades      = B                     # Number of blades
...
Hence, the broadband component seems to over predict relative to the
